...
int  update[6];      //check if a given digit should update
int  change[6];      //check for if change has occured since update
int  key;            //current key press
int  backup_key;     //previous key press
int  stored_key;     //stored key for restoring after paused
int  key_changed;    //Debounce variable to make sure a new key has been pressed
int  cont;           //Check varible to check if method runs multiple times
int  paused;         //Pause check varible
int  reset_flag;     //used to count inputs since last reset, controls num_in
int  buzzed;         //Check variable to make sure buzzer turns off
int  fast = 0;       //Enables fast mode
int  systick_count;  //The delay counter for systick, waits for this number of interupts;
int  used;           //The number of pending updates to the clock face
int  first;          //Check to see if first time through looping function to adjust updates
long counter;        //delay counter for polling keypad
...
/*Experimental segment flip function. Currently Unused*/
...
/* Update handler, Gets 5 systick interrupts and then increments update.*/
/* 1Hz is a little long for systick so it is set to 200mHz*/
...
void count_up()
{
  ...
  if(is_valid()&&(paused == 0)) /*If valid run method*/
  {
    if(first == 1)//wait a second before counting up
    {
      first++;
      cont = 1;
      paused = 0;
      used = 0;   //reset updates, as they pile up before start
    }
    else
    {
      if (time[5] < 9)  //Handles proper count sequencing
      {
         time[5]++;
         update[5] = 1;
      }
   ...
   {
     if(paused == 0)//If not simply paused
     {
        reset();
     }
   }
...
{
  used--;//decrement update counter
  if(is_valid()&&(paused == 0)) /*If valid, run method*/
  {
    if(first == 1)//wait a second before counting down
    {
      first++;
      cont = 1;
      paused = 0;
    ...
    {
      if (time[5] > 0) //Handler for proper count down sequencing
      {
         buzzed = 0;
         time[5]--;
         update[5] = 1;
  ...
  {
    if(paused == 0) //Unless simply paused
    {
      reset();
    }
  }
...
/*Pushes a new number into the LSD and left shifts all other digits then updates
  the clock face*/
...
void push_update(int dir)//updates clock, 0 up, 1 down, 2 force set
{
    ...
    else if (dir == 2) //For forcing a number
    {
      for(int pos =5;pos>=0;pos--)
      {
        if(update[pos] == 1)
...
void keypad() /*Handles the keypad scan function and calls the appropiate function*/
{
  ...
  /*Theory of Operation for scanning:
    ...
    a button push*/
  /*Note that commented out scans and cases are unimplemented or unused buttons*/
  //Scan Column 1
  GPIOA->BSRR = GPIO_Pin_9;//set bit as high
  GPIOA->BRR = GPIO_Pin_10;//set bit as low
  GPIOA->BRR = GPIO_Pin_11;//set bit as low
...
/*    if(GPIO_ReadInputDataBit(GPIOC, GPIO_Pin_4)) //Enter
  ...
  if(((cont == 1)&&(used>0))||((key_changed == 1)&&(counter >= 40000)))
  {
    switch(key)
    {
      case 0:  num_in(1); break;//break; //cycle a unused
      case 1:  num_in(2); break;//break; //cycle b unused
      case 2:  num_in(3); break;//break; //Review memory unused
      case 3:  reset_flag = 10; count_up(); break;//break; //clear entry unused
      ...
      case 7:  num_in(4); break;//break; //[ unused
      case 8:  num_in(5); break; // 7
      case 9:  num_in(6); break; // 4
      case 10: reset_flag = 10; count_down(); break;//num_in(1); break; // 1
      ...
      case 14: num_in(7); break; //break; //] unused
      case 15: num_in(8); break; // 8
      case 16: num_in(9); break; // 5
      case 17: pause(); break;//num_in(2); break; // 2
      ...
      case 22: num_in(0); break; // 9
      case 23: break;//num_in(6); break; // 6
      case 24: reset(); break;//num_in(3); break; // 3
...
int main(void)
{
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_All; /*Control all pins on port*/
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT; /*Set as outputs*/
  ...
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_DOWN; /*Set a pulldown*/
  GPIO_Init(GPIOC, &GPIO_InitStructure); //Set C to input, with pulldowns
  
  SysTick_Config(8000000); /*Set systick rollover value*/
                           /*Bigger = longer delay*/

  reset(); /*Reset clock face*/
  reset(); /*Reset face again to make sure*/
  //  fast = 1;// fast = 1 turns on fast mode, but draws more power.
  while (1)
  {
    keypad(); /*Spin in keypad routine to wait for input*/   
  }
}
