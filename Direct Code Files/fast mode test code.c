        ...
        else
        {
          ...
          else
          {
            ...
            else
            {
              ...
              if(buzzed == 0)
              {
                GPIOB->BSRR = 4;    /* Set PA1 for setting segment*/
                GPIOB->BRR = BSRR_VAL;
                Delay(15000000);
                GPIOB->BSRR = BSRR_VAL;
                GPIOB->BRR = 4;    /* Set PA1 for setting segment*/
                buzzed = 1;
              }
            }
          }
...
void num_in(int new_num)
{
  ...
  if(0 < reset_flag < 7)
  {
    /*for(int times = 0; times < reset_flag+1;times++)
    {
      if(times != 0)
      {
        time[6-reset_flag+times] = time[7-reset_flag+times];
        update[6-reset_flag+times] = 1;
      }
      else
      {
        time[5] = new_num;
 ...
 // if(0 < reset_flag < 7)  /*This statement will implement a set function from left to right*/
 /* {
...
//    clear_digit(32);
  //  count_up();
    //count_down();
    fast = 1;// Fast pulls >2amps when flipping a zero...
    keypad();
    //Delay(6000000);
  }
}
